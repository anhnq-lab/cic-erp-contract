...
import { Search, Filter, Plus, MoreVertical, ExternalLink, User } from 'lucide-react';
import { MOCK_CONTRACTS, MOCK_UNITS, MOCK_SALESPEOPLE } from '../constants';
import { ContractStatus, Unit } from '../types';
    ...
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-600 dark:text-slate-400 text-sm font-bold mt-1">
            Đơn vị: <span className="text-indigo-700 dark:text-indigo-400 font-black uppercase">{selectedUnit.name}</span> 
      ...
      <div className="bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-md flex flex-wrap gap-4 items-center">
        <div className="flex-1 min-w-[240px] relative">
          ...
          <input 
            type="text" 
            placeholder="Tìm mã HĐ, tên khách hàng hoặc nhân viên..." 
        ...
        <table className="w-full text-left border-separate border-spacing-0 min-w-[1400px]">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800 bg-white dark:bg-slate-900">
            {filteredContracts.map((contract, index) => {
              const profit = contract.value - contract.estimatedCost;
              const margin = contract.value > 0 ? (profit / contract.value) * 100 : 0;
              const salesperson = MOCK_SALESPEOPLE.find(s => s.id === contract.salespersonId);
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-5 text-right bg-white dark:bg-slate-900">
                    <span className="text-sm font-black text-slate-900 dark:text-slate-100">
                      {formatCurrency(contract.actualRevenue)}
                    </span>
                  </td>
                  <td className="px-4 py-5 text-right bg-white dark:bg-slate-900">
